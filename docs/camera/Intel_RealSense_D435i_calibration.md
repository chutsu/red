...
- Download the Android app for showing the calibration target from
   [here.](https://play.google.com/store/apps/details?id=com.intel.realsenseviewer17613)

- Run the commands below to install the calibration app on Ubuntu 18.04 (taken
...
- Plug in the camera, launch `Intel.Realsense.DynamicCalibrator` and start the
   calibration target app on the phone.

- Follow the instructions in `Intel.Realsense.DynamicCalibrator`.

- The folder `~/DyCalibResult` contains the calibration results.
...
- Install the dependencies and download the calibration script.
...
- Connect the camera and run the calibration script with `python3
